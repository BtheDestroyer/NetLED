...
def main():
    ...
    s.send(led.Set_Pixel_Packet(0, rpi_ws281x.Color(255,0,0)).to_bytes())
    log.info("Sleeping for 1 second...")
    time.sleep(1)
    log.info("Sending pixel packet 2...")
    s.send(led.Set_Pixel_Packet(1, rpi_ws281x.Color(0,255,0)).to_bytes())
    log.info("Sleeping for 1 second...")
    time.sleep(1)
    log.info("Sending pixel packet 3+4...")
    s.send(led.Set_Pixel_Packet(0, rpi_ws281x.Color(0,0,0)).to_bytes())
    s.send(led.Set_Pixel_Packet(1, rpi_ws281x.Color(0,0,0)).to_bytes())
